...
#'
#' For examples, see [riverLevelData()].
#'
...
#' to work quietly.
#'
#' @value a data frame holding columns named
#' `"id"`, `"name"`, `"latitude"`, `"region"` and `"tz", with
#' one row for each selected station.
#'
#' @author Dan Kelley
riverLevelDirectory <- function(url, station, debug = 0) {
    if (missing(url)) {
...
#' may be discovered by using [riverLevelDirectory ()] first. This
...
#'
#' @saveFile logical value indicating whether to save the file for later
...
#' to work quietly.
#'
#' @value a data frame containing the data, with columns named `"time"` and
#' `"level"`.
#'
#' @examples
#' library(oce)
#' dir <- riverLevelDirectory()
#' data <- riverLevelData(id = dir$id)
...
#'
#' @author Dan Kelley
riverLevelData <- function(id = "01EJ001", region = "NS", interval = "daily", saveFile = FALSE, debug = 0) {
